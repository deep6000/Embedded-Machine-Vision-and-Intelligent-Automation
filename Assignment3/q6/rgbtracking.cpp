 ...
 * @return COM_pts 
 */
COM_pts FindCOM(Mat R,Mat G, Mat B, Cordinates *area);
...
int main()
{
    ...
    Mat frame, bgr[3], g_band,r_band,b_band,final;
	    Mat bgr_prev[3],bgr_curr[3];
    char k;
    int framecount;
    unsigned int i = 0;
    Cordinates spot;
...
if(previous.empty())
        ...
        medianBlur(b_band, b_band, 3);
        medianBlur(g_band, g_band, 3);
        medianBlur(r_band, r_band, 3);

        COM = FindCOM(r_band, b_band,g_band,&spot);
...
COM_pts FindCOM(Mat R,Mat G, Mat B, Cordinates *area)
{
    ...
    for(int row = 1; row < R.rows; row++)
    {
        ...
        {
                if(R.at<uchar>(row,col) > THRESHOLD || G.at<uchar>(row,col) > THRESHOLD  || B.at<uchar>(row,col) > THRESHOLD)
                {
